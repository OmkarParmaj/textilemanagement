...
const Packingprint = () => {
    ...
    const fetchdata = async (Packingslipno, uid) => {
        try {
            const res = await axios.get(`http://api.textilediwanji.com/packingprint/${Packingslipno}/${uid}`, { withCredentials: true });
    ...
    const sendPackslip = () => {
        setLoading(true);
        axios.post("http://api.textilediwanji.com/mailpackslip", { url }, { withCredentials: true })
            .then(() => {
                        ...
                        <div className="container-fluid border border-1" >
                            <div className="row">
                                <div className="col-4 col-md-2 border-bottom border-end d-flex justify-content-center align-items-center ">
                                    {<img className="logomobile" src={`http://api.textilediwanji.com/companyimage/${packingslipData.filenameas}`} style={{ maxWidth: 250, maxHeight: 90 }} alt={`Image ${packingslipData.filenameas}`} ></img> || <Skeleton></Skeleton>}
                                </div>
