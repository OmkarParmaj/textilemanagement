...
const CompanyRegistration = ({ isLoggedIn, setIsLoggedIn }) => {
    ...
    const handleeditfetchdata = (srno) => {
        axios.get(`http://api.textilediwanji.com/handleeditfetchdata/${srno}`, { withCredentials: true })
            .then(res => {
    ...
    const handleeditedsubmit = (e, srno) => {
        ...
        axios.put(`http://api.textilediwanji.com/handleeditedsubmit/${srno}`, values, { withCredentials: true })
            .then(res => {
    ...
    const fetchdata = async () => {
        try {
            setLoading(true)
            const res = await axios.get('http://api.textilediwanji.com/companyreport', { withCredentials: true })
            setComdata(res.data);
            setLoading(false);
    ...
    const handleSubmit = (e) => {
        ...
        axios.post('http://api.textilediwanji.com/companyregister', formData, { withCredentials: true })
            .then(res => {
                setValues({
                    companyname: "",
                    personname: "",
    ...
    const handledelete = (id) => {
        axios.delete(`http://api.textilediwanji.com/companydelete/${id}`, { withCredentials: true })
            .then((res) => {
                fetchdata();
            })
            .catch((err) => {
                                                            ...
                                                            <tbody>
                                                                {comdata.map((company, index) => (
                                                                    <tr key={index}>
                                                                        ...
                                                                        <td><img src={`http://api.textilediwanji.com/companyimage/${company.filenameas}`} style={{ maxWidth: 250, maxHeight: 90 }} alt={` ${index}`} /></td>
